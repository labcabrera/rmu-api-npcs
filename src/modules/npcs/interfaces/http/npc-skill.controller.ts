...
export class NpcSkillController {
  ...
  async addSkill(@Param('id') npcId: string, @Body() dto: AddSkillDto, @Request() req): Promise<NpcDto> {
    this.logger.log(`Adding skill to NPC with ID: ${npcId}`);
    const userId: string = req.user!.id as string;
    const roles: string[] = req.user!.roles as string[];
    const command = AddSkillDto.toCommand(npcId, dto, userId, roles);
  ...
  @HttpCode(HttpStatus.OK)
  async deleteSkill(@Param('id') npcId: string, @Param('skillId') skillId: string, @Request() req): Promise<NpcDto> {
    this.logger.log(`Deleting skill with ID: ${skillId} from NPC with ID: ${npcId}`);
    const userId: string = req.user!.id as string;
    const roles: string[] = req.user!.roles as string[];
    const command = new DeleteSkillCommand(npcId, skillId, userId, roles);
